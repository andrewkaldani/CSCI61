...
namespace main_savitch_10
{
	...
	// to a copy of the removed item.
	{
       ...
       binary_tree_node<Item>*pre = nullptr;
	...
	// Header file used: bintree.h
	{
    binary_tree_node<Item> *cur = root_ptr;
    bool finish = false;
    if(root_ptr==NULL){
      root_ptr = new binary_tree_node<Item>(entry,nullptr);
      return;
    }
    do {
      if(cur->data()>=entry){
        if(cur->left()==NULL){
          cur->set_left(new binary_tree_node<Item>(entry,nullptr));
          finish = true;
        }
        cur = cur->left();
      }
        ...
        if(cur->right()==NULL){
          cur->set_right(new binary_tree_node<Item>(entry,nullptr));
            finish=true;
        }
        cur=cur->right();
      }
